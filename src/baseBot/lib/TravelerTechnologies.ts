...
export const SKILL_NAMES = Object.values(availableProps).map(
  (prop) => prop.skill.name,
);

export const BURST_NAMES = Object.values(availableProps).map(
  (prop) => prop.burst.name,
);
...
export function getTech(options: FindTechArgs) {
  ...
  return kit.techs.find((tech) =>
    tech.id.toLowerCase().includes(options.id.toLowerCase()),
  );
}
