...
export default class BonkUtilities {
  ...
  #crowdSourcedReasons = [
    '"Ratio" - <@462747685242273792>',
    '"Git Gud" - <@440081484855115776>',
  ]
  ...
  isHorny(msg: string = this.message): boolean {
    const notHorny =
      /(n+(o|u)+t*)\s((h+o+r+n+(y|i)+)|(s+e+g+s+)|(s+e+x+))/gimu.test(msg);
    const reallyHorny =
   ...
   */
  bonkReason(): string {
    return this.#allNonHornyReasons[
      Math.floor(Math.random() * this.#reasons.length)
    ];
  }
