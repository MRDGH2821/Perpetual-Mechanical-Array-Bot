...
import { COLORS, ROLE_IDS } from "../../lib/Constants.js";
...
export default class UserCommand extends Subcommand {
  ...
  public async gangMembers(format: "object" | "string" = "string") {
  ...
  public async gangRemove(interaction: Subcommand.ChatInputCommandInteraction) {
    ...
    const gangMembers = (await this.gangMembers("object")) as Collection<
      string,
      GuildMember
    >;
    ...
    return interaction
      ...
      .then(async (i) => {
        if (i.customId === "yes") {
          ...
          gangMembers.forEach(async (member) =>
            member.roles.remove(ROLE_IDS.OTHERS.GANG_GANG_WOO),
          );
