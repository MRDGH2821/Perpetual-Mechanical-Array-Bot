...
import { Client, Formatters } from '@ruinguard/core';

export default class BonkUtilities {
  /**
   * initialises class
   * @constructor
   * @param {object} options - options for bonk utilities
   * @param {string} options.message - the message to check
   * @param {Client} options.client - the client object
   */
  BonkUtilities({ message, client }) {
    this.message = message;
    this.client = client;
  }

  emoji = this.client.emojis.resolve;
  ...
  reasons = [
    ...
    'Ah my hands slipped',
    'Just because...',
    `Because why not ${this.emoji('967344234820681768')}`,
  ];

  hornyReasons = [
    'You need no reason to be told',
    `No Horni ${this.emoji('830243258960838717')}`,
    `No Horni ${this.emoji('830243224105779290')}`,
    `No Segs ${this.emoji('830243224105779290')}`,
    `No Segs ${this.emoji('830243258960838717')}`,
    this.emoji('821169357765345291'),
    '*You are the sole reason why this chat has become impure*',
    '***You shall be bonked***',
  ];

  crowdSourcedHornyReasons = [
    `"Simply un good" - ${Formatters.memberNicknameMention(
      '823564960671072336',
    )}`,
    `"You can do better than that" - ${Formatters.memberNicknameMention(
      '621330211220226049',
    )}`,
    `"Do better" - ${Formatters.memberNicknameMention('621330211220226049')}`,
    `"You suck" - ${Formatters.memberNicknameMention('505390548232699906')}`,
  ];
  ...
  static isHorny(msg) {
    ...
    const reallyHorny = /\bh+o+r+n+(y|i){1,}\b/gimu.test(checkMsg)
