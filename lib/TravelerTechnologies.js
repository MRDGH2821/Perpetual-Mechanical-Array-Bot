...
export async function amcAutoComplete(interaction) {
  ...
  switch (interaction.options.getSubcommand()) {
    ...
    case 'gust_surge': {
      ...
      const values = AMC_TECHS.burstTechs.filter((choice) => choice.name.startsWith(focused));
      responses = values.map((choice) => ({
        name: choice.name,
        value: choice.id,
      }));
...
export async function gmcAutoComplete(interaction) {
  ...
  switch (interaction.options.getSubcommand()) {
    case 'starfell_sword': {
      ...
      const values = GMC_TECHS.skillTechs.filter((choice) => choice.name.startsWith(focused));
      responses = values.map((choice) => ({
        name: choice.name,
        value: choice.id,
      }));
    ...
    case 'wake_of_earth': {
      ...
      const values = GMC_TECHS.burstTechs.filter((choice) => choice.name.startsWith(focused));
      responses = values.map((choice) => ({
        name: choice.name,
        value: choice.id,
      }));
...
export async function emcAutoComplete(interaction) {
  ...
  switch (interaction.options.getSubcommand()) {
    case 'bellowing_thunder': {
      ...
      const values = EMC_TECHS.burstTechs.filter((choice) => choice.name.startsWith(focused));
      responses = values.map((choice) => ({
        name: choice.name,
        value: choice.id,
      }));
